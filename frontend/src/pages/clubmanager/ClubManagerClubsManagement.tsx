...
export const ClubManagerClubsManagement: React.FC = () => {
    const toast = useToast();
    const [clubs, setClubs] = useState<Club[]>(
        allClubs.filter((c) => managedClubIds.includes(c.id))
    );
    ...
    const { register, handleSubmit, reset, formState: { errors } } = useForm<ClubFormData>({
        resolver: zodResolver(clubSchema) as Resolver<ClubFormData>,
    ...
    return (
        <div className="space-y-6">
            <h1 className="text-2xl sm:text-3xl font-bold text-text-primary">My Managed Clubs</h1>
            <p className="text-text-secondary">View and edit the clubs you manage</p>
                ...
                <CardContent>
                    <div className={show(filteredClubs.length === 0)}>
                        <EmptyState
                            ...
                            message="You are not managing any clubs yet."
                        />
                    </div>
                                        ...
                                        <TableCell>
                                            <div className="flex items-center gap-3">
                                                <div className={show(!!club.imageUrl)}>
                                                    <img src={club.imageUrl} alt={club.name} className="h-9 w-9 rounded-lg object-cover shrink-0" />
                                                </div>
                                                <span className="font-medium text-text-primary">{club.name}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <span className="line-clamp-1 text-text-secondary text-sm">{club.description}</span>
                                        </TableCell>
                                        <TableCell>{club.members.toLocaleString()}</TableCell>
                                        <TableCell className="text-right">
                                            <Button variant="ghost" size="sm" onClick={() => handleEdit(club)}>Edit</Button>
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
            ...
            <Modal isOpen={isModalOpen} onClose={closeFormModal} title={editingClub ? 'Edit Club' : 'Add Club'} size="md">
