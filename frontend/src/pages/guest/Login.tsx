...
export const Login: React.FC = () => {
    ...
    const onSubmit = async (data: LoginFormData) => {
        try {
            const loggedInUser = await login(data.username, data.password, data.rememberMe);
            toast.success('Welcome back!');
            const destination =
                loggedInUser.role === 'admin'
                    ? PATHS.admin.dashboard
                    : loggedInUser.role === 'clubmanager'
                        ? PATHS.clubmanager.clubs
                        : PATHS.app.dashboard;
            navigate(destination, {replace: true});
        } catch {
            toast.error('Invalid username or password. Try user/user, admin/admin or clubmanager/clubmanager.');
        }
    };
